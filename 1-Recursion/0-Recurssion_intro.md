...
*What if a function could solve a problem by solving a smaller version of the same problem? That's the idea behind **recursion** one of the most elegant tools in programming.*
...
## 8.6 Recurrence Relation — Time Complexity of Recursion

*We know recursion calls itself repeatedly but how do we calculate its exact time complexity?*

**Recurrence relation** is used to find the **time complexity of recursion**. It is a formula that expresses the time of a function in terms of the time of its smaller call.
...
If **T(n)** is the total time taken by the function, then T(n) equals the **sum of the total number of statements** in the function.
...
**Base case:** When `n = 0`, the `if` condition is false — only that one check runs:

```
T(0) = 1
```

Here **2 is a constant**, so we write it as **m**:
