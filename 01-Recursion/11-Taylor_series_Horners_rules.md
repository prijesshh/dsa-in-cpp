...
> ➕ Added: Horner's Rule can be implemented **recursively** — the innermost bracket `[1 + x/4]` is evaluated first, then each outer bracket uses that result. This makes it a natural fit for a recursive function where each call handles one level of nesting, working from the inside out.

---

## 📚 Quick Recap

- The Taylor series of `e^x` = `1 + x/1! + x²/2! + x³/3! + ...` up to n terms.
- The **naïve computation** evaluates each term independently → **O(n²) multiplications**.
- By **factoring terms** (Horner's Rule), the expression becomes nested brackets evaluated one at a time → **O(n) multiplications**.
- The final factored form for n=4: `1 + x/1 [1 + x/2 [1 + x/3 [1 + x/4]]]`
- This nested structure maps directly to **recursion** — each recursive call handles one bracket level.